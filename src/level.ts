...
export class Level {
  ...
  constructor(public world: CANNON.World, public scene: THREE.Scene, public rngSeed = Math.random() * 1e9) {
    ...
    const floorMesh = new THREE.Mesh(new THREE.PlaneGeometry(100, 100), new THREE.MeshStandardMaterial({ color: 0xa9b49f }))
  ...
  generate(spec: LevelSpec) {
    ...
    for (let i = 0; i < spec.roomCount - 1; i++) {
      ...
      if (size.x > size.z) {
        ...
        const left = new THREE.Box3(box.min.clone(), new THREE.Vector3(mid - 0.2, box.max.y, box.max.z))
        const right = new THREE.Box3(new THREE.Vector3(mid + 0.2, box.min.y, box.min.z), box.max.clone())
        remaining.push(left, right)
      } else {
        // split along Z
        const mid = box.min.z + size.z * randRange(0.35, 0.65)
        const near = new THREE.Box3(box.min.clone(), new THREE.Vector3(box.max.x, box.max.y, mid - 0.2))
        const far = new THREE.Box3(new THREE.Vector3(box.min.x, box.min.y, mid + 0.2), box.max.clone())
    ...
    const wallMat = new THREE.MeshStandardMaterial({ color: 0xdedede, roughness: 0.9 })
    ...
    const drawRoomWalls = (box: THREE.Box3, openings: THREE.Vector3[]) => {
      ...
      for (const [a, b] of edges) {
        ...
        const doorCenter = new THREE.Vector3().addVectors(a, b).multiplyScalar(0.5)
        // If an opening is requested close to this edge, leave a gap
        const gap = openings.find(o => new THREE.Vector3().subVectors(o, doorCenter).length() < 0.5)
        if (gap) {
    ...
    for (let i = 0; i < rooms.length; i++) {
      ...
      for (let j = 0; j < rooms.length; j++) {
        ...
        const touchX = overlapX > 0 && (Math.abs(box.max.z - other.min.z) < 0.05 || Math.abs(box.min.z - other.max.z) < 0.05)
        const touchZ = overlapZ > 0 && (Math.abs(box.max.x - other.min.x) < 0.05 || Math.abs(box.min.x - other.max.x) < 0.05)
        if (touchX || touchZ) {
          // place doorway center within overlapping span
          if (touchX) {
            const z = (Math.max(box.min.z, other.min.z) + Math.min(box.max.z, other.max.z)) / 2
            const x = Math.abs(box.max.x - other.min.x) < 0.1 ? box.max.x : box.min.x
            openings.push(new THREE.Vector3(x, 0, z))
          } else if (touchZ) {
            const x = (Math.max(box.min.x, other.min.x) + Math.min(box.max.x, other.max.x)) / 2
            const z = Math.abs(box.max.z - other.min.z) < 0.1 ? box.max.z : box.min.z
            openings.push(new THREE.Vector3(x, 0, z))
          }
        }
      }
      ...
      if (Math.random() < 0.5 && Math.abs(box.max.z - 10) < 0.2) {
        openings.push(new THREE.Vector3((box.min.x + box.max.x)/2, 0, box.max.z))
      }
    ...
    const fence = (a: THREE.Vector3, b: THREE.Vector3) => {
      ...
      mesh.position.copy(new THREE.Vector3().addVectors(a, b).multiplyScalar(0.5)); mesh.position.y = fenceH/2
      mesh.rotation.y = angle; this.scene.add(mesh); this.meshes.push(mesh)
    ...
    fence(new THREE.Vector3(yardMin.x,0,yardMin.z), new THREE.Vector3(yardMax.x,0,yardMin.z))
    ...
    const yardMesh = new THREE.Mesh(new THREE.BoxGeometry(yardSize.x, 0.1, yardSize.z), new THREE.MeshStandardMaterial({ color: 0xa8c789 }))
    yardMesh.position.set((yardMin.x + yardMax.x)/2, 0.05, (yardMin.z + yardMax.z)/2)
    yardMesh.receiveShadow = true
    this.scene.add(yardMesh); this.meshes.push(yardMesh)
    const yardBody = new CANNON.Body({ mass: 0, shape: new CANNON.Box(new CANNON.Vec3(yardSize.x/2, 0.05, yardSize.z/2)) })
    yardBody.position.set(yardMesh.position.x, yardMesh.position.y, yardMesh.position.z)
    this.world.addBody(yardBody); this.bodies.push(yardBody)
    ...
    const placeBox = (pos: THREE.Vector3, size: THREE.Vector3, mat: THREE.Material, mass = 0) => {
      ...
      if (mass > 0) { body.angularDamping = 0.2; body.linearDamping = 0.01 }
      this.world.addBody(body); this.bodies.push(body)
      return mesh
    }
    ...
    for (const r of rooms) {
      ...
      // Counter
      if (Math.random() < 0.8) {
        placeBox(new THREE.Vector3(cx, 0.9, cz), new THREE.Vector3(randRange(2.5,3.5), 0.9, randRange(1.0, 1.6)), choice(furnMats))
      }
      // Table
      if (Math.random() < 0.6) {
        placeBox(new THREE.Vector3(cx + randRange(-1.5, 1.5), 0.75, cz + randRange(-1.5, 1.5)), new THREE.Vector3(1.2, 0.6, 1.2), choice(furnMats))
      }
      // Couch
      if (Math.random() < 0.5) {
        placeBox(new THREE.Vector3(cx + randRange(-2,2), 0.6, cz + randRange(-2,2)), new THREE.Vector3(2.0, 0.8, 0.8), choice(furnMats))
      }
    }

    // Clutter: dynamic physics objects on top of furniture
    const furnitureSurfaces = this.meshes.filter(m => m instanceof THREE.Mesh && (m as THREE.Mesh).geometry instanceof THREE.BoxGeometry && m.position.y > 0.5)
    for (let i = 0; i < spec.clutterCount; i++) {
      const surf = choice(furnitureSurfaces) as THREE.Mesh
      if (!surf) break
      const size = new THREE.Vector3().fromArray((surf.geometry as THREE.BoxGeometry).parameters) as any
      const px = randRange(surf.position.x - size.width/2 + 0.1, surf.position.x + size.width/2 - 0.1)
      const pz = randRange(surf.position.z - size.depth/2 + 0.1, surf.position.z + size.depth/2 - 0.1)
      const p = new THREE.Vector3(px, surf.position.y + (surf as any).geometry.parameters.height/2 + 0.1, pz)
      const w = randRange(0.1, 0.3)
      placeBox(p, new THREE.Vector3(w, w, w), new THREE.MeshStandardMaterial({ color: 0xffffff * Math.random() }), randRange(0.1, 0.6))
    }
  }
}
