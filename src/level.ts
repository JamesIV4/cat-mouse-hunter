...
export class Level {
  ...
  update(dt: number) {
    for (const p of this.dynamicPairs) {
      ...
      (p.mesh as any).quaternion &&
        (p.mesh as any).quaternion.set(bq.x, bq.y, bq.z, bq.w);
    }
  }
  ...
  generate(spec: LevelSpec) {
    ...
    const drawRoomWalls = (box: THREE.Box3, openings: THREE.Vector3[]) => {
      ...
      for (const [a, b] of edges) {
        ...
        if (chosenCenter) {
          ...
          const rightStart = chosenCenter
            .clone()
            .addScaledVector(dir, doorHalf);
          // Clamp to segment
    ...
    if (backIdx >= 0) {
      ...
      roomOpenings[backIdx].push(
        new THREE.Vector3((r.min.x + r.max.x) / 2, 0, r.max.z)
      );
    }
    ...
    const makeCouch = (length: number, depth = 0.9, height = 0.8) => {
      ...
      const base = new THREE.Mesh(
        new THREE.BoxGeometry(length, 0.08, depth),
        wood
      );
      ...
      const seat = new THREE.Mesh(
        new THREE.BoxGeometry(length - armW * 2, seatH, depth - 0.06, 2, 1, 2),
        fabric
      );
      ...
      const back = new THREE.Mesh(
        new THREE.BoxGeometry(length - armW * 2, backH, 0.12),
        fabric2
      );
      back.position.set(
        0,
        legH + 0.08 + seatH + backH / 2,
        -(depth / 2) + 0.06
      );
      ...
      const armL = new THREE.Mesh(
        new THREE.BoxGeometry(armW, height, depth),
        fabric2
      );
      ...
      for (let i = 0; i < cushionCount; i++) {
        const w =
          (length - armW * 2 - 0.06 * (cushionCount + 1)) / cushionCount;
        const c = new THREE.Mesh(
          new THREE.BoxGeometry(w, seatH * 0.95, depth - 0.08, 2, 1, 2),
          fabric
        );
        c.position.set(
          -(length / 2 - armW) + 0.06 + w / 2 + i * (w + 0.06),
          legH + 0.08 + seatH / 2 + 0.01,
          0.01
        );
      ...
      for (const o of openings) {
        ...
        if (Math.abs(o.z - room.max.z) < eps) {
          const min = new THREE.Vector2(
            o.x - (doorHalf + widthExtra),
            room.max.z - depthClear
          );
        ...
        if (Math.abs(o.x - room.max.x) < eps) {
          const min = new THREE.Vector2(
            room.max.x - depthClear,
            o.z - (doorHalf + widthExtra)
          );
          const max = new THREE.Vector2(
            room.max.x,
            o.z + (doorHalf + widthExtra)
    ...
    const placeCouchAlongWall = (
      ...
      const length =
        (randRange(1.6, 3.2) + sizeFactor * randRange(0.4, 1.0)) * scale;
      const depth =
        (randRange(0.8, 1.0) + sizeFactor * randRange(0.0, 0.2)) * scale;
      ...
      while (tries-- > 0) {
        if (wall === 0) {
          ...
          yaw = Math.PI;
          if (
            !wouldBlockDoor(room, openings, wall, x, length / 2) &&
            !rectIntersects(x, z, length / 2, depth / 2)
          ) {
            foundPos = true;
            break;
          }
          ...
          yaw = -Math.PI / 2;
          if (
            !wouldBlockDoor(room, openings, wall, z, length / 2) &&
            !rectIntersects(x, z, depth / 2, length / 2)
          ) {
            foundPos = true;
            break;
          }
      ...
      if (wouldBlockDoorCorridor(room, openings, x, z, yaw, length, depth))
        return 0;
      ...
      {
        const halfXMain =
          Math.abs(Math.sin(yaw)) > 0.5 ? depth / 2 : length / 2;
        const halfZMain =
          Math.abs(Math.sin(yaw)) > 0.5 ? length / 2 : depth / 2;
        couchRects.push({ x, z, halfX: halfXMain, halfZ: halfZMain });
      }
      ...
      if (longSpan > 6 && Math.random() < 0.5 + sizeFactor * 0.3) {
        ...
        for (const endSign of [1, -1]) {
          ...
          for (const s of [1, -1]) {
            const targetCorner = pCorner
              .clone()
              .sub(F.clone().multiplyScalar(gap));
            ...
            if (
              c2.x - len2 / 2 < room.min.x + 0.5 ||
              c2.x + len2 / 2 > room.max.x - 0.5
            )
              continue;
            if (
              c2.z - dep2 / 2 < room.min.z + 0.5 ||
              c2.z + dep2 / 2 > room.max.z - 0.5
            )
              continue;
            if (
              wouldBlockDoorCorridor(
                room,
                openings,
                c2.x,
                c2.z,
            ...
            {
              const halfX2 =
                Math.abs(Math.sin(yaw2)) > 0.5 ? dep2 / 2 : len2 / 2;
              const halfZ2 =
                Math.abs(Math.sin(yaw2)) > 0.5 ? len2 / 2 : dep2 / 2;
    ...
    for (let ri = 0; ri < rooms.length; ri++) {
      if (Math.random() < 0.7)
        totalCouches += placeCouchAlongWall(rooms[ri], roomOpenings[ri]);
      if (Math.random() < 0.35)
        totalCouches += placeCouchAlongWall(rooms[ri], roomOpenings[ri]);
    }
    let safety = 50;
    ...
    for (let i = 0; i < spec.clutterCount; i++) {
      ...
      if (Math.random() < 0.5) {
        ...
        const h = baseH * scale;
        const mesh = new THREE.Mesh(
          new THREE.CylinderGeometry(rr, rr, h, 16),
          mat
        );
