...
function createLevel(n: number) {
  ...
  // mice
  for (const m of mice) { if (m.alive) m.kill() }
  mice = []
  const spec = specForLevel(n)
  required = spec.miceRequired
  for (let i = 0; i < spec.mouseCount; i++) {
    const p = level.spawnPoints[Math.floor(Math.random()*level.spawnPoints.length)] || new THREE.Vector3(0,0,0)
...
function loop() {
  ...
  // update entities
  cat.update(dt, camera, mice.filter(m => m.alive).map(m => m.mesh))
  ...
  for (const m of mice) {
    ...
    if (d < 0.6 && (cat.state === 'Pounce' || cat.state === 'Run' || cat.state === 'Walk' || cat.state === 'Sneak')) {
      m.kill()
      caught++
      remaining--
      UI.setCaught(caught)
      ...
      if (caught >= required && !bannerVisible) {
        UI.showBanner(`<h2>House cleared!</h2><p>You caught ${caught} mice.</p><p>Press <b>N</b> for the next house.</p>`)
        bannerVisible = True
      }
    }
  }
